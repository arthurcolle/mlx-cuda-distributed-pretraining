#!/bin/bash
# Script to run a quick training test without flash attention
...
python train.py --config model-config-test.yaml --log-interval 1 > "runs/test-${RUN_ID}.log" 2>&1

echo "Test complete. Check log at runs/test-${RUN_ID}.log"

# Quick verification of the log file
if [ -f "runs/test-${RUN_ID}.log" ]; then
  echo "Test log contents:"
  head -n 10 "runs/test-${RUN_ID}.log"
  echo "..."
  tail -n 5 "runs/test-${RUN_ID}.log"
fi