...
run_experiment() {
  ...
  # Create a temporary config with the specific optimizer and run name
  cp model-config-1m.yaml "model-config-1m-${optimizer}.yaml"
  sed -i.bak "s/name: \"Micro-1M\"/name: \"${run_name}\"/" "model-config-1m-${optimizer}.yaml"
  sed -i.bak "s/optimizer: \"muon\"/optimizer: \"$optimizer\"/" "model-config-1m-${optimizer}.yaml"
  sed -i.bak "s/learning_rate: 3.0e-3/learning_rate: $lr/" "model-config-1m-${optimizer}.yaml"
  ...
  rm "model-config-1m-${optimizer}.yaml"
  rm "model-config-1m-${optimizer}.yaml.bak"
}
...
echo "Compare results with: python plot_fixed.py \"runs/Micro-1M-*\""